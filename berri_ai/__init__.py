                ...
                if isinstance(child, ast.Assign):
                    ...
                    if key_val in tree_dict: 
                        if node_value_source == tree_dict[key_val]:
                            ...
                            raise KeyError(
                                f"🚨🚨 Deployment Error 📣: The variable - {key_val}"
                                + ' has been initialized 2 times. Here are the instances: \n 1.'
                                + tree_dict[key_val][:100]
                                + '\n 2.'
                                + value_source[:100]
                            )
                    ...
                    for kv in key_val:
                        ...
                        if kv in tree_dict: 
                            if node_value_source == tree_dict[kv]:
                                ...
                                raise KeyError(
                                    f"🚨🚨 Deployment Error 📣: The variable - {kv}"
                                    + ' has been initialized 2 times. Here are the instances: \n 1.'
                                    + tree_dict[kv][:100]
                                    + '\n 2.'
                                    + value_source[:100]
        ...
        try:
            ...
            if match := re.search(r"'(.*)'", text):
                ...
                install_statements = [
                    "import subprocess",
                    str(
                        f"""subprocess.call(['pip', 'install','{package_name}'])"""
                    ),
                ]
...
def copy_files():
    ...
    if not os.path.exists(f'{current_directory}/berri_files'):
        os.mkdir(f'{current_directory}/berri_files')
    ...
    for subdir, dirs, files in os.walk(current_directory):
        for file in files:
            if (
                file not in ['sample_data', 'drive']
                and subdir == current_directory
            ):
                        # print("file: ", file)
                shutil.copy(file, f'{current_directory}/berri_files')
        for dir in dirs:
            if (dir not in list_of_restricted_dirs) and (subdir == current_directory):
                        # print("dir: ", dir)
                shutil.copytree(dir, f'{current_directory}/berri_files/{dir}')
...
def install_requirements():
    ...
    for package_name in existing_reqs:
        install_statements = [
            "import subprocess",
            str(f"""subprocess.call(['pip', 'install','{package_name}'])"""),
        ]
        for install_statement in install_statements:
          exec(install_statement)
...
def deploy_func(user_email: str, executing_function, test_str: str):
    ...
    try:
        ...
        agent_executing_line = f"{str(executing_function)}('{test_str}')"
        ...
        parent_dependencies = [
            value for key, value in tree_dict.items() if "os" in key
        ]
        ...
        print(f"🚧 Currently deploying to [NOT READY YET] 👉 {endpoint}")
        print(f"⌛️ It'll be ready in 15 mins. We'll email you  @ {user_email}")
    except Exception as e:
      print(e)
      traceback.print_exc()
        ...
        with open("./berri_files/agent_code.py", "w") as f:
            for line in lines:
                if ".query(" in line:
                    ...
                    print("line: ", line)
                    initialization_line = line.split("=", 1)[1] if "=" in line else line
                    ...
                    reinitialize_agent = f"agent = {prev_name}"
  ...
  # replace with new name 
        ...
        with open("./berri_files/agent_code.py", "w") as f:
            for line in lines:
                if (
                    original_agent_name
                    and f"{original_agent_name}.run(" in line
                ): # if it exists and is being executed, replace that line
                    line = line.replace(f"{original_agent_name}.run(", "agent.run(")
                    f.write(line + "\n")
                if "initialize_agent(" in line:
                    ...
                    original_agent_name = initialization_line[0].strip() # get the original agent variable name
                            # print("original_agent_name: ", original_agent_name)
                    initialization_line = f"agent = {initialization_line[1]}"
                    # print(initialization_line)
